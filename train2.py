...
def train(datasets, labels ,roar, judge):
    ...
    for curr_fold in range(fold):
        ...
        for epoch in range(training_epochs):
            ...
            PATH='./param2/model_{}_{}.pth'.format(roar,judge)
            torch.save(model.state_dict(),PATH)
        total_train_acc.append(train_acc)
        total_train_loss.append(train_loss)
        total_test_acc.append(test_acc)
        total_test_loss.append(test_loss)
    with open("./data2/train_loss_{}_{}.pickle".format(roar,judge),mode="wb") as f:
        pickle.dump(total_train_loss,f)
    with open("./data2/test_loss_{}_{}.pickle".format(roar,judge),mode="wb") as f:
        pickle.dump(total_test_loss,f)
    with open("./data2/train_acc_{}_{}.pickle".format(roar,judge),mode="wb") as f:
        pickle.dump(total_train_acc,f)
    with open("./data2/test_acc_{}_{}.pickle".format(roar,judge),mode="wb") as f:
...
def data_32_mean(data_origin, rank, roar_level):
    ...
    data = torch.randn(32,32,720,1,384)
...
def data_32_zero(data_origin, rank, roar_level):
    ...
    data_origin = torch.torch.from_numpy(data_origin).clone()
    data_origin = data_origin.permute(2,0,3,1)
    data_zero = torch.zeros(720,1,384)
    if rank == None:
        for i in range(32):
            data_i = data_origin.clone()
            data_i[i] = data_zero
